...
export function EditPatientPageContent({ patientId }: EditPatientPageContentProps) {
    ...
    const effectivePatientId = patientId ?? searchParams.get('id') ?? undefined;
    ...
    useEffect(() => {
        if (effectivePatientId) {
            const load = async () => {
                try {
                    const data = await patientService.getPatientById(effectivePatientId);
                    if (data) {
                        setPatient(data);
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                router.push(`/patients/edit?id=${encodeURIComponent(foundPatient.id ?? '')}`);
            } else {
