...
import { useAvailableEntities } from '../../hooks/useAvailableFilters';
...
interface TestReportFiltersProps {
...
export function TestReportFilters({
    ...
    onClear,
    isLoading = false,
}: TestReportFiltersProps) {
    const now = new Date();
    const currentYear = now.getFullYear();
    const currentMonth = now.getMonth();
    const years = Array.from({ length: 5 }, (_, i) => currentYear - i);
    const { options: entityOptions } = useAvailableEntities();
                ...
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-neutral-900">
                            Reporte de oportunidad por prueba
                        </h3>
                        <p className="text-sm text-neutral-500">
                            Seleccione mes, año y entidad para generar el informe.
                        </p>
                    </div>
                </div>
            </div>
