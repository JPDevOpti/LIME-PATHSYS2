...
export function TestsOpportunityPanel({ prueba }: TestsOpportunityPanelProps) {
    // totalProcedures = unidades de prueba (quantity; alineado con informe Pruebas).
    const totalProcedures = prueba.totalProcedures;
    const unEvaluated = Math.max(
        0,
        totalProcedures - prueba.withinOpportunity - prueba.outOfOpportunity,
    );
    const completion =
        totalProcedures > 0
            ? Math.round((prueba.withinOpportunity / totalProcedures) * 100)
            : 0;
            ...
            <div className="p-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                    ...
                    <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-200">
                        <div className="text-xs text-neutral-600 mb-1">Total unidades de esta prueba</div>
                        <div className="text-xl font-semibold text-neutral-700">{totalProcedures}</div>
                        {unEvaluated > 0 && (
                            <div className="text-[11px] text-neutral-500 mt-1">
                                Sin evaluación oportunidad: {unEvaluated}
                            </div>
                        )}
                    </div>
                </div>
