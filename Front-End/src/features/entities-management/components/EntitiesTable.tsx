...
import { ClipboardPen, CheckCheck, Ban } from 'lucide-react';
...
interface EntitiesTableProps {
    ...
    onInactivate: (entity: Entity) => void;
        ...
        return (
            <BaseCard className="p-6">
                <div className="text-center py-12 text-neutral-500">
                    <p className="text-sm">{noResultsMessage ?? 'No hay entidades creadas. Cree una nueva para comenzar.'}</p>
                </div>
            </BaseCard>
        );
    }
                        ...
                        {entities.map((e) => (
                            <tr key={e.id} className="hover:bg-neutral-50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span className="text-sm text-neutral-700 font-mono">{e.code}</span>
                                </td>
                                <td className="px-6 py-4 max-w-[200px]">
                                    <span className="text-sm text-neutral-700 line-clamp-2">{e.observations || '-'}</span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <span
                                        className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                            e.is_active ? 'bg-green-100 text-green-800' : 'bg-neutral-100 text-neutral-600'
                                        }`}
                                    >
