...
from app.modules.patients.schemas.patient import PatientResponse, EntityInfo, build_patient_code
...
class FakeService:
    async def change_patient_identification(self, patient_code: str, new_identification_type, new_identification_number: str):
        return PatientResponse(
            ...
            patient_code=build_patient_code(new_identification_type, new_identification_number),
...
def test_change_identification_route_success():
    ...
    resp = client.put(
        "/patients/CC-111/change-identification",
        params={"new_identification_type": 1, "new_identification_number": "22222"},
    )

    assert resp.status_code == 200
    body = resp.json()
    assert body["patient_code"] == "CC-22222"
