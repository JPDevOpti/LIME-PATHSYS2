...
from app.modules.patients.schemas.patient import IdentificationType, PatientResponse, EntityInfo, Gender, CareType, build_patient_code
...
class FakeRepo:
    ...
    async def change_identification(self, old_code, new_type, new_number, cases_collection):
        return {
            ...
            "patient_code": build_patient_code(new_type, new_number),
...
async def test_service_change_identification_accepts_enum_int_str(monkeypatch):
    ...
    # Caso 1: Enum
    resp = await service.change_patient_identification("CC-111", IdentificationType.CEDULA_CIUDADANIA, "22222")
    assert isinstance(resp, PatientResponse)
    assert resp.patient_code == "CC-22222"

    # Caso 2: int
    resp2 = await service.change_patient_identification("CC-111", 1, "33333")
    assert resp2.patient_code == "CC-33333"

    # Caso 3: str
    resp3 = await service.change_patient_identification("CC-111", "1", "44444")
    assert resp3.patient_code == "CC-44444"