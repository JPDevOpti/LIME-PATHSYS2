...
IDENTIFICATION_TYPE_TO_CODE = {
    IdentificationType.CEDULA_CIUDADANIA.value: "CC",
    IdentificationType.CEDULA_EXTRANJERIA.value: "CE",
    IdentificationType.TARJETA_IDENTIDAD.value: "TI",
    IdentificationType.PASAPORTE.value: "PA",
    IdentificationType.REGISTRO_CIVIL.value: "RC",
    IdentificationType.DOCUMENTO_EXTRANJERO.value: "DE",
    IdentificationType.NIT.value: "NIT",
    IdentificationType.CARNET_DIPLOMATICO.value: "CD",
    IdentificationType.SALVOCONDUCTO.value: "SC",
    IdentificationType.NN.value: "NN",
    IdentificationType.AS.value: "AS",
    IdentificationType.PT.value: "PT",
}

IDENTIFICATION_CODE_TO_TYPE = {v: k for k, v in IDENTIFICATION_TYPE_TO_CODE.items()}


def normalize_identification_type_value(identification_type: IdentificationType | int | str) -> int:
    if isinstance(identification_type, IdentificationType):
        return int(identification_type.value)

    if isinstance(identification_type, int):
        if identification_type in IDENTIFICATION_TYPE_TO_CODE:
            return identification_type
        raise ValueError("Tipo de identificación inválido")

    if isinstance(identification_type, str):
        value = identification_type.strip().upper()
        if not value:
            raise ValueError("Tipo de identificación inválido")

        if value.isdigit():
            numeric = int(value)
            if numeric in IDENTIFICATION_TYPE_TO_CODE:
                return numeric
            raise ValueError("Tipo de identificación inválido")

        if value in IDENTIFICATION_CODE_TO_TYPE:
            return IDENTIFICATION_CODE_TO_TYPE[value]

    raise ValueError("Tipo de identificación inválido")


def identification_type_to_code(identification_type: IdentificationType | int | str) -> str:
    normalized_type = normalize_identification_type_value(identification_type)
    return IDENTIFICATION_TYPE_TO_CODE[normalized_type]
...
def normalize_identification_number(value: str) -> str:
    ...
    if not normalized.isalnum():
        raise ValueError("El número de identificación solo puede contener letras y números")
...
def build_patient_code(identification_type: IdentificationType | int | str, identification_number: str) -> str:
    return f"{identification_type_to_code(identification_type)}-{normalize_identification_number(identification_number)}"
...
class PatientBase(BaseModel):
    ...
    def validate_identification_number(cls, v):
        return normalize_identification_number(v)
...
class PatientUpdate(BaseModel):
    ...
    identification_number: Optional[str] = Field(None, min_length=5, max_length=20)
...
class PatientSearch(BaseModel):
    ...
    identification_type: Optional[IdentificationType] = None
    identification_number: Optional[str] = Field(None, min_length=1, max_length=20)
    ...
    def normalize_identification_number(cls, v):
        ...
        if not value:
            return None
        return normalize_identification_number(value)
