...
- Enums:
  - IdentificationType (int): tipos de documento (1=CC, 2=CE, 3=TI, 4=PA, 5=RC, 6=DE, 7=NIT, 8=CD, 9=SC, 10=NN, 11=AS, 12=PT).
  - Gender (str): Masculino, Femenino.
  - CareType (str): Ambulatorio, Hospitalizado.

## Validaciones claves
- identification_number: alfanumérico (A-Z, 0-9), longitud entre 5 y 20.
...
## Reglas de negocio
- `patient_code` se compone como: `CODIGO_DOCUMENTO` + "-" + `identification_number` (ej.: `CC-12345`, `NN-ABC123`).
- Cambio de identificación: actualiza `identification_type`, `identification_number` y recomputa `patient_code` usando el código de documento en letras.
