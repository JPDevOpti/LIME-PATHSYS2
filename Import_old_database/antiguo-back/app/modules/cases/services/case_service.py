...
from app.modules.patients.schemas import build_patient_code
...
class CaseService:
            ...
            if patient_info:
                ...
                if id_type and id_number:
                    if not patient_info.get("patient_code"):
                        try:
                            patient_info["patient_code"] = build_patient_code(id_type, id_number)
                        except ValueError:
    ...
    def _to_response(self, doc: Dict[str, Any]) -> CaseResponse:
        ...
        if not normalized_patient.get("patient_code") and id_type and id_number:
            try:
                normalized_patient["patient_code"] = build_patient_code(id_type, id_number)
            except ValueError:
