...
Para cada patólogo:
  - Si ya existe en local (por email) → actualiza signature + password_hash
...
def run(dry_run: bool, dest_url: str = LOCAL_URL, dest_db: str = LOCAL_DB):
    ...
    for u in dst_users.find({"role": "pathologist"}):
        local_users_by_email[u.get("email", "").lower()] = u
    print(f"  Patólogos ya en local: {len(local_users_by_email)}")
    ...
    for path in src_pathos.find({}):
        ...
        # ── Caso 1: ya existe en local → actualizar signature + password_hash ─
        if email in local_users_by_email:
            ...
            try:
                ...
                print(f"  [UPDATE] {email:40} → signature + hash sincronizados")
                stats["updated"] += 1
    ...
    print("RESULTADO")
    print(f"{'─'*62}")
    print(f"  Actualizados (signature+hash) : {stats['updated']}")
