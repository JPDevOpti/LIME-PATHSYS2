...
def build_pathologist(raw: dict | None) -> dict | None:
    if not raw:
        return None
    return {
        "id":   str(raw.get("id", "")),
...
def transform_case(
    ...
    if old.get("assigned_resident"):
        ap = build_pathologist(old["assigned_resident"])
        if ap:
    ...
    doc = {
        ...
        "assigned_pathologist":       build_pathologist(old.get("assigned_pathologist")),
...
def run(
    ...
    for old in src_cases.find({}).batch_size(batch_size):
        ...
        doc = transform_case(old, local_pt, now, tests_time_map)
